...
// 		var $error = this.$error,$loginForm = $('#loginForm');
// 		var user = {
...
// 		var $starttime = this.$starttime,$duringtime = this.$duringtime;
...
// 			if( $(e.target).data('action') != undefined )e.preventDefault();
...
// 			 	console.log('1');
// 			if( $this.attr('class') =='login' || $this.attr('class') =='reg' || $.contains( $shawdow[0],e.target )){
// 			}else{
// 				if( $shawdow )
// 				$shawdow.hide().find('input').not('input[type="button"]').val('');
// 				$shawdow.find('.error,.errorL').hide();
...
// 			if( $this.attr('id') == 'apt-sub' || $this.attr('id') == 'apt-fail' || $aptFail[0]  == e.target || $.contains( $aptFail[0],e.target ) ){
